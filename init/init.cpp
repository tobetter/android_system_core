...
void service::NotifyStateChange(const char* new_state) {
    ...
    init_property_set(prop_name, new_state);
}
...
static int console_init_action(int nargs, char **args)
{
    ...
    if (init_property_get("ro.boot.console", console) > 0) {
...
static void process_kernel_dt(void)
{
    ...
    while ((dp = readdir(dir.get())) != NULL) {
        ...
        init_property_set(property_name.c_str(), dt_file.c_str());
    }
}
...
int main(int argc, char** argv) {
    ...
    if (init_property_get("ro.bootmode", bootmode) > 0 && strcmp(bootmode, "charger") == 0) {
